...
func (kc *KVCreate) check() error {
	if _, ok := kc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`local: missing required field "KV.created_at"`)}
	}
	if _, ok := kc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`local: missing required field "KV.updated_at"`)}
	}
	if _, ok := kc.mutation.Key(); !ok {
		return &ValidationError{Name: "key", err: errors.New(`local: missing required field "KV.key"`)}
	}
	if _, ok := kc.mutation.Value(); !ok {
		return &ValidationError{Name: "value", err: errors.New(`local: missing required field "KV.value"`)}
	}
	return nil
}
...
func (u *KVUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("local: missing options for KVCreate.OnConflict")
	}
...
func (u *KVUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("local: OnConflict was set for builder %d. Set it on the KVCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("local: missing options for KVCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}

...
func (kq *KVQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("local: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (kq *KVQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range kq.inters {
		if inter == nil {
			return fmt.Errorf("local: uninitialized interceptor (forgotten import local/runtime?)")
		}
	...
	for _, f := range kq.ctx.Fields {
		if !kv.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("local: invalid field %q for query", f)}
		}
	}
	if kq.path != nil {

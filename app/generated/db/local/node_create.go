...
func (nc *NodeCreate) check() error {
	if _, ok := nc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`local: missing required field "Node.created_at"`)}
	}
	if _, ok := nc.mutation.Name(); !ok {
		return &ValidationError{Name: "name", err: errors.New(`local: missing required field "Node.name"`)}
	}
	if _, ok := nc.mutation.IP(); !ok {
		return &ValidationError{Name: "ip", err: errors.New(`local: missing required field "Node.ip"`)}
	}
	if _, ok := nc.mutation.Port(); !ok {
		return &ValidationError{Name: "port", err: errors.New(`local: missing required field "Node.port"`)}
	}
	if _, ok := nc.mutation.Comment(); !ok {
		return &ValidationError{Name: "comment", err: errors.New(`local: missing required field "Node.comment"`)}
	}
	return nil
}
...
func (u *NodeUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("local: missing options for NodeCreate.OnConflict")
	}
...
func (u *NodeUpsertOne) ID(ctx context.Context) (id uuid.UUID, err error) {
	if u.create.driver.Dialect() == dialect.MySQL {
		...
		return id, errors.New("local: NodeUpsertOne.ID is not supported by MySQL driver. Use NodeUpsertOne.Exec instead")
	}
...
func (u *NodeUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("local: OnConflict was set for builder %d. Set it on the NodeCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("local: missing options for NodeCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}

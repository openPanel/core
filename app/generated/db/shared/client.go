...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	if _, ok := c.driver.(*txDriver); ok {
		return nil, errors.New("shared: cannot start a transaction within a transaction")
	}
	tx, err := newTx(ctx, c.driver)
	if err != nil {
		return nil, fmt.Errorf("shared: starting a transaction: %w", err)
	}
	cfg := c.config
	cfg.driver = tx
	return &Tx{
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("shared: unknown mutation type %T", m)
	}
}
...
func (c *KVClient) mutate(ctx context.Context, m *KVMutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("shared: unknown KV mutation op: %q", m.Op())
	}
}
...
func (c *NodeClient) mutate(ctx context.Context, m *NodeMutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("shared: unknown Node mutation op: %q", m.Op())
	}
}

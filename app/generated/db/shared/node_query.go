...
func (nq *NodeQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("shared: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (nq *NodeQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range nq.inters {
		if inter == nil {
			return fmt.Errorf("shared: uninitialized interceptor (forgotten import shared/runtime?)")
		}
	...
	for _, f := range nq.ctx.Fields {
		if !node.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("shared: invalid field %q for query", f)}
		}
	}
	if nq.path != nil {
